...
int UghRecursion(int x, int y, const vector<vector<bool> >& blocked_grid, vector<vector<int> >& history) {
  if (blocked_grid[x][y]==false) {
    ...
    if ((x==0)&&(y==0)) {
      return 1;
        ...
        int sum = 0 ;
        if (x>0) {
          sum+=UghRecursion(x-1,y,blocked_grid,history);
        }
        if (y>0) {
          sum+=UghRecursion(x,y-1,blocked_grid,history);
        }
      return sum;
    }
  }
...
int main(int argc, char* argv[]) {
  ...
  // g++ start.cpp -o Ugh.exe -Wall

  std::cout << "Starting cooridinates are x="<<start_x<<" and y="<<start_y<<std::endl;
  int numPaths = UghRecursion(start_x,start_y,blocked_grid,history);
  std::cout << std::endl << "The number of paths to return to the origin is: " << numPaths << std::endl;

  std::cout << std::endl << "One set of correct steps to the origin is:" << std::endl;

  unsigned int startSplice = 0;
  unsigned int endSplice = 0;

  unsigned int endPoint = 0;

  for (unsigned int i=0;i<history.size();i++) {
    //std::cout << "("<<history[i][0]<<", "<<history[i][1]<<")" << std::endl;
    if (i>2) {
    ...
    if ((history[i][0]==0)&&(history[i][1])==0) {
      endPoint=i;
      break;
    }
  }
  ...
  for (unsigned int i=0;i<endPoint+1;i++) {
    if ((history[i][0]==history[endSplice][0])&&((history[i][1]-1)==history[endSplice][0])) {
      startSplice = i;
    }
  }

  //std::cout << history[startSplice][0] << " " <<history[startSplice][1];


  for (unsigned int i=0;i<startSplice;i++) {
    std::cout << "("<<history[i][0]<<", "<<history[i][1]<<") to" << std::endl;
  }
  for (unsigned int i=endSplice;i<endPoint+1;i++) {
    std::cout << "("<<history[i][0]<<", "<<history[i][1]<<") to" << std::endl;
  }
