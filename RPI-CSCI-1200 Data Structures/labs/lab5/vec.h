...
template <class T> typename Vec<T>::size_type remove_matching_elements(Vec<T>& v,const T& val);
...
template <class T> void Vec<T>::resize(size_type n, const T& fill_in_value) {
  if (n <= m_size) {
    m_size = n;
    // Reduce allocation if necessary
    if (m_size<=(m_alloc/2)) {
      m_alloc=n;
      T* new_data = new T[m_alloc];
      for (size_type i =0;i<m_size;i++) {
...
template <class T>  void Vec<T>::print() const {
  std::cout << "\nThe value of m_alloc is: " << m_alloc << std::endl;
  std::cout << "The value of m_size is: " << m_size << std::endl;
  std::cout << "The value of m_data is: " << m_data << std::endl;
