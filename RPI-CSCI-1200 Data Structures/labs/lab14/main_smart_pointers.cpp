...
void addToSet(Balloon* root, std::set<Balloon*>& all) {

  

  for (unsigned int i=0;i<root->getRope(i)->numRopes();i++) {

    if (root->getRope(i)->getName()=="Scooby Doo") {
    std::cout << "Scooby Doo here" << std::endl;
    for (unsigned int i=0;i<root->numRopes();i++) {
      std::cout << root->getRope(i)->getName() << std::endl;
    }
  }
    all.insert(root->getRope(i));
    if (all.find((root->getRope(i)))==all.end()) {
      addToSet(root->getRope(i),all);
    }
    all.insert(root);
  }
