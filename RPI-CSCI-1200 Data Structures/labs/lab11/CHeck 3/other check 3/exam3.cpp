...
const T& FindLargestInTree(TreeNode<T>* root){
  ...
  if (root->right!=NULL) {
    return FindLargestInTree(root->right);
  } else {
    return root->value;
  }
}
...
std::vector<T> TreeSort(TreeNode<T>* root){
  ...
  TreeNode<T>* find = FindSmallestInRange(return_vector[return_vector.size()-1],end,root);
  while(find){
    return_vector.push_back(find->value);
    find = FindSmallestInRange(return_vector[return_vector.size()-1],end,root);
  }
