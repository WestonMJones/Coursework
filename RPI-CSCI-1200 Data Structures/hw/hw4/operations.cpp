...
float multidivide(int numerator, int d1, int d2, int d3, int d4) {
  double d4n = (double)d4;
  float f = ((((numerator / d1) / d2) / d3) / d4n);
  return f;
}
...
int list_operations() {
  ...
  int count = 0;
  for(std::list<char>::reverse_iterator ritr = l1.rbegin(); ritr != l1.rend(); ritr++) {
    if(*ritr < 'a' || *ritr > 'z') {
      break;
    }
    count++;
...
bool vector_compare(const std::vector<int> v1, const std::vector<int> v2) {
  uint counter = std::min(v1.size(),v2.size());
  bool success = true;
