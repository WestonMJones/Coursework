...
void iplc_sim_push_pipeline_stage()
{
    ...
    if (pipeline[DECODE].itype == BRANCH) {
		...
		/* if(pipeline[ALU].itype == RTYPE){
			if(pipeline[DECODE].stage.branch.reg1 == pipeline[ALU].stage.rtype.dest_reg){
				
			}
			if(pipeline[DECODE].stage.branch.reg2 == pipeline[ALU].stage.rtype.dest_reg){
    ...
    if (pipeline[MEM].itype == LW) {
		...
		if(pipeline[ALU].itype == RTYPE){
			//if regs are being used in the destination and source registers
			int same_dest_reg1 = pipeline[ALU].stage.rtype.reg1 == pipeline[MEM].stage.lw.dest_reg;
			int same_dest_reg2 = pipeline[ALU].stage.rtype.reg2_or_constant == pipeline[MEM].stage.lw.dest_reg;
			// if conflict
			if (same_dest_reg1 || same_dest_reg2){
				//instert nop
				inserted_nop++;
				//increment cycle
				pipeline_cycles++;
				//copies stage back
				memcpy(&pipeline[WRITEBACK], &pipeline[MEM], sizeof(pipeline_t));
				//nops the current stage
				bzero(&pipeline[MEM], sizeof(pipeline_t));
			}
		}
		
		//if miss
		if(!iplic_sim_trap_address(pipeline[MEM].instruction_address)){
			//stall
	...
	//cycles through each stage
	for(i = MAX_STAGES - 1; i > FETCH; i--){
		//copies data over (can't set using = )
...
void iplc_sim_process_pipeline_syscall()
{
    //fill syscall values -- just instruction and address
	iplc_sim_push_pipeline_stage();
...
void iplc_sim_process_pipeline_nop()
{
    //fill nop values -- just instruction and address
	iplc_sim_push_pipeline_stage();
