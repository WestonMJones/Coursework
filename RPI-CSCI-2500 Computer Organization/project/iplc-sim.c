...
void iplc_sim_init(int index, int blocksize, int assoc)
{
    ...
    for (i = 0; i < (1<<index); i++) {
        cache[i].valid_bit = calloc(assoc,sizeof(int));
        cache[i].tag = calloc(assoc,sizeof(int));
        cache[i].usage_history = calloc(assoc,sizeof(int));

        for (int count = 0;count<assoc;count++) {
            cache[i].valid_bit[count] = 0;
            cache[i].tag[count] = 0;
            cache[i].usage_history[count] = 0;
        }
...
void iplc_sim_LRU_replace_on_miss(int index, int tag)
{
    ...
    // There was a miss so update the counter
    cache_miss++;

    // Create a variable to indicate which block we're going to access
    // Default is the first
    int block_accessed = 0;


    // Loop through the usage history's of all the blocks at that index
    for (int i=0;i<cache_assoc;i++) {

        // Find the block that hasn't been used in the longest time
        if (cache[index].usage_history[i]>cache[index].usage_history[block_accessed]) {
            block_accessed=i;
        }
    }

    // Update the usage history of all blocks at that index
    for (int i=0;i<cache_assoc;i++) {
        cache[index].usage_history[i]++;
    }

    // Update the values of the chosen block
    cache[index].usage_history[block_accessed] = 0;
    cache[index].valid_bit[block_accessed] = 1;
    cache[index].tag[block_accessed] = tag;
...
void iplc_sim_LRU_update_on_hit(int index, int assoc_entry)
{
    ...
    // Loop through the blocks at that index, incrementing their usage counts
    for (int i=0;i<cache_assoc;i++) {
        cache[index].usage_history[i]++;
    }

    // Reset the usage count of the block we just accessed
    cache[index].usage_history[assoc_entry]=0;
}
...
int iplc_sim_trap_address(unsigned int address)
{
    ...
    // Delete the offset from the address
    int address_copy = address >> cache_blockoffsetbits;

    // Create a mask to splice the index off of the address
    int mask = 1 << (cache_index);
    mask = mask - 1;

    // Grab the index
    index = mask & address_copy;

    // The tag is everything that's left
    tag = address_copy >> cache_index;

    // Print everything out
    printf("Address %x: Tag= %x, Index= %d \n",address,tag,index);

    // Loop through the associated blocks at that index
    for (i=0;i<cache_assoc;i++) {

        // If two tags equal and the valid bit it true, it's a hit
        if ((cache[index].tag[i]==tag) && 
            (cache[index].valid_bit[i]==1)) {
                hit++;
                cache_hit++;

                // Update usage_history for that block
                iplc_sim_LRU_update_on_hit(index, i);             
        }
    }

    // Increment cache accesss as well
    cache_access++;


    // If there was a miss, call the replace function
    if (hit==0) {
...
void iplc_sim_push_pipeline_stage()
{
    ...
    if (pipeline[DECODE].itype == BRANCH) {
        ...
        if (pipeline[FETCH].instruction_address!=0) {  
            ...
            // Handle the accuracy of our predictions
            if (branch_taken == branch_predict_taken){
                // Guessed correctly, so increment appropriate counter
                correct_branch_predictions++;
            } else {
                // Bad prediction, so incur a stall penalty
                pipeline_cycles++;
            }
        }
    }
    ...
    if (pipeline[MEM].itype == LW) {
        ...
        // First check for data misses
        data_hit = iplc_sim_trap_address(pipeline[MEM].stage.lw.data_address);

        // Check for register conflicts
        if ((pipeline[ALU].stage.lw.dest_reg == pipeline[MEM].stage.sw.src_reg) || // OR
            (pipeline[ALU].stage.lw.dest_reg == pipeline[MEM].stage.lw.dest_reg)) {

            // If there is a conflict, increment cycles and insert a nop
            pipeline_cycles++;
            inserted_nop = 1; 

            // Account for the registers
            pipeline[WRITEBACK] = pipeline[MEM];
            bzero(&(pipeline[MEM]), sizeof(pipeline_t));
        }

        // If there's a miss, then add 9 cycles (not 10 because we're already pushing down pipelne)
        if (data_hit == 0){
            
            pipeline_cycles = pipeline_cycles + 9;

            // Also account for the added nop
            pipeline_cycles = pipeline_cycles - inserted_nop;
    ...
    if (pipeline[MEM].itype == SW) {
        ...
        if (iplc_sim_trap_address(pipeline[MEM].stage.sw.data_address)==0) {
            // If the there's a miss, then add 9 cycles (not 10 because we're already pushing down pipelne)
            pipeline_cycles = pipeline_cycles + 9;
        }
    ...
    pipeline[DECODE] = pipeline[FETCH]; // Instructor comment above is a typo, should be FETCH->DECODE
...
void iplc_sim_process_pipeline_lw(int dest_reg, int base_reg, unsigned int data_address)
{
    ...
    // Set the correct instruction type
    pipeline[FETCH].itype = LW;

    // Use the global instruction_address
    pipeline[FETCH].instruction_address = instruction_address; 

    // Fill in the internal variables with the correct values from the parameters
...
{
    /* You must implement this function */
    // DO the same as before -- yay this part is easyyyyy
...
void iplc_sim_process_pipeline_jump(char *instruction)
{
    ...
    // Use strcpy to copy the instruction (character array) correctly
