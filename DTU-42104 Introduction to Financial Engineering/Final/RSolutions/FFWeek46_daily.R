#Replication using internet data
FF_data = read.table("C:/Users/nilan/Dropbox/IFE2019/Exercises/Rstuff/FamaFrenchDaily.txt", header = TRUE) #replace with relevant link
...
getSymbols(c('GE', 'IBM', 'XOM'), src = 'yahoo', periodicity = 'daily', return.class="data.frame", from = '1963-06-28',to='2010-05-29')
CalcReturns = function(prices){
  return( diff(prices) / prices[-length(prices)])
}
ge = CalcReturns(GE$GE.Adjusted)*100 - RF
ibm = CalcReturns(IBM$IBM.Adjusted)*100 - RF
mobil = CalcReturns(XOM$XOM.Adjusted)*100 - RF
